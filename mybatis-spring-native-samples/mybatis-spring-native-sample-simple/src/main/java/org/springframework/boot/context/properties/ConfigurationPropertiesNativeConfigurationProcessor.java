...
/**
 * A {@link BeanFactoryNativeConfigurationProcessor} that allows reflection access on all declared methods of
 * {@link ConfigurationProperties @ConfigurationProperties} annotated types, their nested types and any complex types
 * that are exposed as a sub- namespace.
 *
 * Workaround class until fixed for https://github.com/spring-projects-experimental/spring-native/pull/1420 (Will remove
 * this class at future)
 *
...
class ConfigurationPropertiesNativeConfigurationProcessor implements BeanFactoryNativeConfigurationProcessor {
  ...
  /**
   * Process a given type for binding purposes, discovering any nested type it may expose via a property.
   */
  private static class TypeProcessor {
    ...
    private void process(NativeConfigurationRegistry registry) {
      ...
      reflection.withAccess(TypeAccess.DECLARED_METHODS, TypeAccess.PUBLIC_METHODS); // Flag.allPublicMethods required
                                                                                     // to handle inherited methods
    ...
    private boolean isClassOnlyReflectionType() {
      return this.type.getPackageName().startsWith("java.") || this.type.isArray()
          || this.type.equals(ApplicationContext.class) || this.type.equals(Environment.class);
    }
    ...
    private Constructor<?> findBindingConstructor() {
      ...
      if (this.constructorBinding) {
        List<Constructor<?>> candidates = Arrays.stream(allConstructors).filter(TypeProcessor::hasConstructorBinding)
